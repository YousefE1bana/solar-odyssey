...
struct CelestialBodyData {
    ...
    float realDiameterKm;          // Real physical diameter in km
    float relativeSizeToEarth;     // Relative diameter compared to Earth (Earth = 1.0)
    float distanceFromSunAU;       // Distance in Astronomical Units (AU)
    float distanceFromSunMillionKm;// Distance in million km
    float orbitalPeriodDays;       // Orbital period in Earth days
    float rotationPeriodHours;     // Rotation period around own axis (hours, negative if retrograde)
    float axialTiltDeg;            // Axial tilt in degrees
    int knownMoons;                // Confirmed moons count
    float surfaceGravityMs2;       // Surface gravity in m/s^2 (Earth = 9.8)
    float meanTemperatureC;        // Mean surface/cloud-top temperature in Celsius
    ...
    glm::vec3 themeColor;          // Accent RGB color for UI badge/highlights (0..1)
    
    // Visualization rendering parameters (matches project scale)
    float visualSize;              // Visual radius in scene units
    float visualOrbitRadius;       // Visual orbital radius in scene units
    float visualSpinSpeed;         // Visual spin speed
    float visualOrbitSpeed;        // Visual orbit speed
