...
enum MissionObjectiveType {
    OBJ_REACH_TARGET = 0,       // Fly within threshold distance of target body
    OBJ_ENTER_ORBIT = 1,        // Engage orbit assist around target body
    OBJ_PROXIMITY_FLYBY = 2,    // High speed flyby within close range
    OBJ_PHOTOGRAPH = 3,         // Capture a photo in photo mode with target body
    OBJ_VISIT_BLACK_HOLE = 4,   // Travel to the deep-space Black Hole
    OBJ_TRAVERSE_WORMHOLE = 5,  // Fly through the Wormhole portal
    OBJ_GRAND_TOUR = 6          // Sequential multi-waypoint expedition
};

struct Mission {
    int id;
    std::string title;
    std::string category;       // e.g. "Navigation", "Orbital Recon", "Astrophotography", "Deep Space"
...
public:
    ...
    void initMissions() {
        ...
        // 4. Mission 4: Ringmaster Snapshot
        Mission m4;
        m4.id = 4;
        ...
        // 6. Mission 6: Einstein-Rosen Bridge
        Mission m6;
        m6.id = 6;
    ...
    void selectNextMission() {
        ...
        // If all completed, just advance normally
    ...
    void completeMission(Mission& m) {
        ...
        // Auto-advance to next incomplete mission if possible
        selectNextMission();
    }
    ...
    void update(float dt, const glm::vec3& shipPos, float shipSpeed, int flightMode,
        ...
        // Resolve the authoritative distance to a named body: prefer the nearest-body
        // measurement, fall back to the selected-target measurement ONLY if the target
        // name matches. Returns false when neither source refers to the requested body,
        // preventing unrelated bodies' distances from corrupting objective progress.
        ...
        case OBJ_ENTER_ORBIT: {
            ...
            if (resolveTargetDist(activeM->targetName, dist)) {
                if (dist <= activeM->targetDistanceThreshold) {
                    ...
                    if (isOrbiting || flightMode == 1 /* FLIGHT_ORBIT_ASSIST */) {
                        completeMission(*activeM);
                    }
                } else {
