/*
 * Spaceship System for Solar Odyssey
 * 
 * Features:
 * - 6-DOF inertial flight physics with smooth acceleration, deceleration, and warp boost
 * - Multiple camera perspectives: Chase, Close-Follow, and Cockpit (1st-person POV)
 * - Intelligent Orbit Assist and planetary intercept autopilot
 * - Proximity sensing and soft repulsive collision safety envelope
 * - Sleek procedural sci-fi spacecraft geometry with dynamic ion thruster exhaust and particle trails
 */
...
enum SpaceshipCameraView {
    SHIP_CAM_CHASE = 0,    // Third-person chase camera with smooth spring follow
    SHIP_CAM_CLOSE = 1,    // Closer third-person tactical follow
    SHIP_CAM_COCKPIT = 2   // First-person cockpit forward view
};

enum SpaceshipFlightMode {
    FLIGHT_MANUAL = 0,     // Full pilot manual 6-DOF flight
    FLIGHT_ORBIT_ASSIST = 1,// Automated circular orbit around nearest/target planet
    FLIGHT_AUTOPILOT = 2   // Waypoint intercept towards target planet
};
...
public:
    ...
    // Transform & Motion
    glm::vec3 position = glm::vec3(0.0f, 6.0f, 22.0f); // Default spawn near Earth/Mars zone
    glm::vec3 prevFramePosition = position;            // Position at end of last update (anti-tunneling)
    ...
    // Flight Parameters
    float throttle = 0.0f;          // Target throttle: -0.4 (reverse) to 1.0 (forward)
    float currentThrottle = 0.0f;   // Smoothed actual throttle
    bool isBoosting = false;
    float boostEnergy = 100.0f;     // Boost meter (0 .. 100)
    float maxBoostEnergy = 100.0f;
    float boostRechargeRate = 18.0f;
    float boostDrainRate = 35.0f;

    float cruiseSpeed = 40.0f;      // Max normal cruise speed (units/s)
    float boostSpeed = 140.0f;      // Warp boost speed (units/s)
    float acceleration = 32.0f;     // Forward accel rate
    float brakeDecel = 40.0f;       // Braking deceleration rate
    float linearDrag = 0.982f;      // Space inertial damping per frame
    ...
    // Target Planet & Proximity Sensing
